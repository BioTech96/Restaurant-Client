...
export const getItems = createAsyncThunk("items/getItems", async (_id) => {
  try {
    const response = await axios.get("https://restaurant-server-5.onrender.com/getItems");
...
export const addItems = createAsyncThunk("items/addItems", async (itemData) => {
  try {
    ...
    const response = await axios.post("https://restaurant-server-5.onrender.com/addItems", {
      name: itemData.name,
...
export const updateItems = createAsyncThunk(
  ...
  async (priceData) => {
    try {
      const response = await axios.post("https://restaurant-server-5.onrender.com/updateItems", {
        name: priceData.name,
...
export const deleteItems = createAsyncThunk(
  ...
  async (postid) => {
    try {
      await axios.delete(`https://restaurant-server-5.onrender.com/deleteItems/${postid}`);
      return postid;
    } catch (error) {
      console.log(error);
    }
