...
export const getItems = createAsyncThunk("items/getItems", async (_id) => {
  try {
    const response = await axios.get("http://127.0.0.1:3002/getItems");
...
export const addItems = createAsyncThunk("items/addItems", async (itemData) => {
  try {
    ...
    const response = await axios.post("http://127.0.0.1:3002/addItems", {
      name: itemData.name,
...
export const updateItems = createAsyncThunk(
  ...
  async (priceData) => {
    try {
      const response = await axios.post("http://127.0.0.1:3002/updateItems", {
        name: priceData.name,
...
export const deleteItems = createAsyncThunk(
  ...
  async (postid) => {
    try {
      await axios.delete(`http://127.0.0.1:3002/deleteItems/${postid}`);
      return postid;
    } catch (error) {
      console.log(error);
    }
