...
export const completeOrder = createAsyncThunk(
  ...
  async (orderId, { rejectWithValue }) => {
    try {
      const response = await axios.post("https://restaurant-server-5.onrender.com/completeOrder", {
        orderId,
      });
...
export const fetchOrders = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get("https://restaurant-server-5.onrender.com/getOrders");
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        "https://restaurant-server-5.onrender.com/getCompletedOrders"
      );
      return response.data.orders;
    } catch (error) {
      return rejectWithValue(error.response.data);
